        ...
        <div className="flex min-h-screen">
            ...
            <div className="from-primary via-primary/90 to-primary/80 relative hidden flex-col justify-between overflow-hidden bg-gradient-to-br p-12 lg:flex lg:w-1/2">
                ...
                <div className="text-primary-foreground relative z-10 space-y-6">
                    <h1 className="text-5xl leading-tight font-bold">{t?.login?.owner?.heroTitle}</h1>
                    <p className="text-xl opacity-90">{t?.login?.owner?.heroSubtitle}</p>
                    <div className="flex gap-8 pt-8">
                        <div>
                            <div className="text-4xl font-bold">500+</div>
                            <div className="text-sm opacity-80">{t?.login?.owner?.stats?.listed}</div>
                        </div>
                        <div>
                            <div className="text-4xl font-bold">€2M+</div>
                            <div className="text-sm opacity-80">{t?.login?.owner?.stats?.contracts}</div>
                        </div>
                        <div>
                            <div className="text-4xl font-bold">98%</div>
                            <div className="text-sm opacity-80">{t?.login?.owner?.stats?.satisfaction}</div>
                        </div>
                    </div>
                </div>

                <div className="text-primary-foreground/80 relative z-10 text-sm">{t?.login?.owner?.copyright}</div>
            </div>
                    ...
                    <Card className="border-2 p-8 shadow-xl">
                        <div className="mb-8 text-center">
                            ...
                            <h2 className="text-foreground mb-2 text-3xl font-bold">
                                {isLogin ? t?.login?.owner?.welcomeBack : t?.login?.owner?.createAccount}
                            </h2>
                            <p className="text-muted-foreground">
                                {isLogin ? t?.login?.owner?.subtitleLogin : t?.login?.owner?.subtitleRegister}
                            </p>
                        </div>
                        ...
                        <div className="mt-6 text-center text-sm">
                            <span className="text-muted-foreground">
                                {isLogin ? t?.login?.owner?.noAccount : t?.login?.owner?.haveAccount}
                            </span>{" "}
                            <button onClick={() => setIsLogin(!isLogin)} className="text-primary font-semibold hover:underline">
                                {isLogin ? t?.login?.owner?.register : t?.login?.owner?.login}
                            </button>
                        </div>

                        <div className="mt-6 border-t pt-6 text-center">
                            <p className="text-muted-foreground mb-4 text-sm">{t?.login?.owner?.promoterQuestion}</p>
                            <Link href="/login/promotor">
                                <Button variant="outline" className="w-full bg-transparent">
                                    {t?.login?.owner?.accessAsPromoter}
                                </Button>
                            </Link>
                        </div>
                    </Card>
