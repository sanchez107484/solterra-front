...
import { Calendar, Euro, Eye, Landmark, MapPin, MessageCircle, Ruler, TrendingUp } from "lucide-react"
...
const getTerrenoStateLabel = (estado: string) => {
    switch (estado?.toUpperCase()) {
        case "ACTIVO":
            return "Activo"
        case "VENDIDO":
...
export default function MisTerrenosPage() {
    ...
    return (
        <>
            <DashboardHeader
                title={t?.dashboard?.owner?.terrenos?.title || "Mis Terrenos"}
                breadcrumbs={[
                ...
                <div className="mb-8 grid grid-cols-1 gap-6 md:grid-cols-4">
                        ...
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-sm font-medium text-gray-600 dark:text-gray-400">
                                    {t?.dashboard?.owner?.terrenos?.stats?.total || "Total Terrenos"}
                                </p>
                    ...
                    <Card className="p-6">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-sm font-medium text-gray-600 dark:text-gray-400">
                                    {t?.dashboard?.owner?.terrenos?.stats?.active || "Activos"}
                                </p>
                    ...
                    <Card className="p-6">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-sm font-medium text-gray-600 dark:text-gray-400">
                                    {t?.dashboard?.owner?.terrenos?.stats?.sold || "Vendidos"}
                                </p>
                    ...
                    <Card className="p-6">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-sm font-medium text-gray-600 dark:text-gray-400">
                                    {t?.dashboard?.owner?.terrenos?.stats?.totalValue || "Valor Total"}
                                </p>
                ...
                {isLoading ? (
                    ...
                    <Empty>
                        <EmptyHeader>
                            ...
                            <EmptyTitle>{t?.dashboard?.owner?.terrenos?.empty?.title || "No tienes terrenos"}</EmptyTitle>
                            <EmptyDescription>
                                {t?.dashboard?.owner?.terrenos?.empty?.description ||
                                    "Añade tu primer terreno para comenzar a buscar proyectos compatibles"}
                            </EmptyDescription>
                        </EmptyHeader>
                        <EmptyContent>
                            <Link href="/dashboard/propietario/nuevo-terreno">
                                <Button>{t?.dashboard?.owner?.terrenos?.empty?.action || "Añadir Terreno"}</Button>
                            </Link>
                        </EmptyContent>
                    </Empty>
                ) : (
                                ...
                                <div className="p-6">
                                    ...
                                    <div className="mb-4 flex items-start justify-between">
                                        <div className="flex-1">
                                            <h3 className="mb-2 text-lg font-semibold text-gray-900 dark:text-white">{terreno.titulo}</h3>
                                    ...
                                    <div className="mb-4 space-y-3">
                                        ...
                                        {terreno.precioVenta && (
                                            <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-300">
                                                ...
                                                <span className="font-semibold text-green-600">{formatCurrency(terreno.precioVenta)}</span>
                                            </div>
                                        )}
                                        ...
                                        <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-300">
                                            ...
                                            <span>
                                                {t?.dashboard?.owner?.terrenos?.created || "Creado"}:{" "}
                                    ...
                                    {/* Descripción */}
                                    {terreno.descripcion && (
                                        <p className="mb-4 line-clamp-2 text-sm text-gray-600 dark:text-gray-300">{terreno.descripcion}</p>
                                    )}
                                    ...
                                    <div className="flex gap-2 border-t pt-4">
                                        ...
                                        <Button variant="outline" size="sm">
                                            ...
                                            {t?.dashboard?.owner?.terrenos?.contact || "Contactar"}
                                        </Button>
                                    </div>
                                </div>
                            </Card>
