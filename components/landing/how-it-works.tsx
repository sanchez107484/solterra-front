          ...
          {steps.map((step, index) => (
            <div key={index} className="relative">
              <div className="flex flex-col items-center text-center">
                ...
                <p className="text-pretty text-muted-foreground">{step.desc}</p>
              </div>

              {/* Connector line */}
              {index < steps.length - 1 && (
                <div className="absolute left-1/2 top-8 hidden h-0.5 w-full bg-gradient-to-r from-primary to-secondary lg:block" />
              )}
            </div>
          ))}
        </div>
